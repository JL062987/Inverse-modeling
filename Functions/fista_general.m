function [X, iter, runtime, min_cost] = fista_general(grad, proj, Xinit, L, opts, calc_F)   
...
%     opts = initOpts(opts);
    ...
    if ~isfield(opts,"Display_Details")
        opts.Display_Details = false;
    end
    ...
    while  iter < opts.max_iter
        iter = iter + 1;
        x_new = feval(proj, y_old - Linv*feval(grad, y_old), lambdaLiv);
        ...
        if e < opts.tol
            runtime = toc(t_start);
            str = strcat("FISTA: ",num2str(iter) ," iterations; time: ",num2str(runtime,'%3.3f'),'.');
            if opts.Display_Details; disp(str); end
            break;
        end
        %% update
        x_old = x_new;
